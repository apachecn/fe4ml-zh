...
只有当数据为正值时, Box-Cox 公式才能工作。对于非正则数据, 可以通过加上固定常量来移动数值。当应用Box-Cox 变换或更一般的幂变换时, 我们必须确定参数λ的值。这可能是通过最大可能 (找到的λ, 最大限度的高斯可能性的结果转换信号) 或贝叶斯方法。对Box-Cox 和一般功率变换的使用的充分处理超出了本书的范围。感兴趣的读者可能会发现更多关于计量经济学方法的权力转换的信息, 由杰克. 约翰斯顿和约翰 DiNardo (麦格劳希尔)。幸运的是, Scipy 的数据包包含了一个框-Cox 转换的实现, 其中包括查找最佳变换参数。
